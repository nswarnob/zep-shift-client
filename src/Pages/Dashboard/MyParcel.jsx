    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 to-white py-12 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          {[
            { label: "Total Parcels", value: parcels.length, icon: "📦", color: "from-blue-50 to-blue-100", textColor: "text-blue-600" },
            { label: "Delivered", value: parcels.filter((p) => p.status === "delivered").length, icon: "✅", color: "from-green-50 to-green-100", textColor: "text-green-600" },
            { label: "In Transit", value: parcels.filter((p) => p.status === "shipped").length, icon: "🚚", color: "from-purple-50 to-purple-100", textColor: "text-purple-600" },
            { label: "Pending", value: parcels.filter((p) => p.status === "pending" || p.status === "processing").length, icon: "⏳", color: "from-yellow-50 to-yellow-100", textColor: "text-yellow-600" },
          ].map((stat, idx) => (
            <div
              key={idx}
        ...
        {filteredParcels.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center animate-slideUpFade border-2 border-gray-100">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">No Parcels Found</h3>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-gray-100 to-gray-50 border-b-2 border-gray-200">
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Parcel Info</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Route</th>
                    <th className="px-6 py-4 text-left text-sm font-bold text-gray-700">Cost</th>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div>
                            ...
                            <p className="text-sm text-gray-600">{parcel.parcelWeight} kg • {parcel.parcelType}</p>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <p className="font-bold text-lg text-lime-600">৳ {parcel.cost || "N/A"}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span
                ...
                return (
                  ...
                  >
                    <div className="mb-4 flex items-start justify-between">
                      <div>
                        <p className="font-bold text-lg text-gray-900">{parcel.parcelName}</p>
                        <p className="text-sm text-gray-600">{parcel.parcelWeight} kg • {parcel.parcelType}</p>
                      </div>
                      <span
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <p className="font-bold text-lime-600 text-lg">৳ {parcel.cost || "N/A"}</p>
                      </div>
                      <div>
        ...
        {/* Footer Info */}
        <div className="mt-12 bg-gradient-to-r from-lime-50 to-blue-50 rounded-2xl p-8 border-2 border-lime-200 animate-slideUpFade" style={{ animationDelay: "0.2s" }}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="text-4xl mb-2">🎯</div>
              <h3 className="font-bold text-gray-900 mb-2">Track in Real-Time</h3>
              <p className="text-sm text-gray-600">Monitor your parcels every step of the way</p>
            </div>
            <div className="text-center">
              <div className="text-4xl mb-2">📱</div>
              <h3 className="font-bold text-gray-900 mb-2">Get Updates</h3>
              <p className="text-sm text-gray-600">Receive SMS and email notifications</p>
            </div>
            <div className="text-center">
              <div className="text-4xl mb-2">💬</div>
